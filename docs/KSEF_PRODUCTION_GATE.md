...
|---|---|---|---|---|
| 1 Runtime | Boots on PHP 8.5; all migrations apply; the eight `pl_ksef_*` tables, their unique constraints and indexes exist; wizard, status page, Sales Orders KSeF block and invoice panel render; settings save; health check runs; scheduler/queue can poll. | `sudo bash bin/ksef-runtime-check.sh /srv/accounting/foundation` (mechanical rows) + the HUMAN rows it lists, done logged in. No framework-free test substitutes. | prerequisite for 4–20 | mechanical **PASS 2026-09-08** — 43 rows, 0 failed, run by Shyam on vmi3221804 at commit `dbd73ab` (`/tmp/ksef-gate1.log`): boots on PHP 8.5, migration `2026_09_08_000100` ran, eight tables + unique constraints + indexes, commands, routes, Filament page in both panels, health NOT CONNECTED with non-zero exit, scheduler/queue units active, HTTP surface answers. **4 HUMAN rows open** (screens, logged in). |
